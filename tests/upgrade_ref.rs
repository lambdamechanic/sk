...
fn upgrade_ref_override_persists_without_commit_change() {
    ...
    write(&work.join("skill").join("SKILL.md"), "---\nname: s0\ndescription: test\n---\n");
    ...
    let mut archive = Command::new("git")
        .args(["-C", cache.to_str().unwrap(), "archive", "--format=tar", &v1, "skill"]) 
    ...
    let digest_v1 = {
        ...
        let mut files: Vec<_> = walkdir::WalkDir::new(&dest)
            .into_iter().filter_map(|e| e.ok()).filter(|e| e.file_type().is_file()).map(|e| e.into_path()).collect();
        files.sort();
        for p in files { 
    ...
    let out = cmd.current_dir(&project)
        ...
        .args(["upgrade", "s0", "--ref", "new-branch"]).output().unwrap();
    assert!(out.status.success(), "upgrade failed: {out:?}");

    // Lockfile should record ref=new-branch even if commit unchanged
    let new_lock: serde_json::Value = serde_json::from_str(&fs::read_to_string(project.join("skills.lock.json")).unwrap()).unwrap();

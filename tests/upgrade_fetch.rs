...
fn upgrade_does_not_mutate_on_extract_failure() {
    ...
    let cache0 = clone_to_cache(&cache_root, host, owner, "r0", &bare0);
    ...
    let cache1 = clone_to_cache(&cache_root, host, owner, "r1", &bare1);
    ...
    let lock = serde_json::json!({
        "version":1,
        "skills":[
            {"installName":"s0","source": {"url":"file://dummy","host":host,"owner":owner,"repo":"r0","skillPath":"skill-0"},"ref": null,"commit": v1_0,"digest": dig0,"installedAt":"1970-01-01T00:00:00Z"},
            {"installName":"s1","source": {"url":"file://dummy","host":host,"owner":owner,"repo":"r1","skillPath":"skill-1"},"ref": null,"commit": v1_1,"digest": dig1,"installedAt":"1970-01-01T00:00:00Z"}
        ],
        "generatedAt":"1970-01-01T00:00:00Z"
    });
    write(&project.join("skills.lock.json"), &serde_json::to_string_pretty(&lock).unwrap());

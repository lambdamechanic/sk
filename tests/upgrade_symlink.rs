...
fn upgrade_preserves_symlinks() {
    ...
    fs::write(work.join("skill/SKILL.md"), "---\nname: s0\ndescription: test\n---\n").unwrap();
    git(&["add", "."], &work);
    git(&["commit", "-m", "v1"], &work);
    git(&["push", "-u", "origin", "main"], &work);
    // v2: append to real.txt
    fs::OpenOptions::new().append(true).open(work.join("skill/real.txt")).unwrap().write_all(b"v2\n").unwrap();
    git(&["add", "."], &work);
    ...
    git(&["clone", bare.to_str().unwrap(), cache.to_str().unwrap()], cache.parent().unwrap());
    ...
    let mut archive = Command::new("git")
        .args(["-C", cache.to_str().unwrap(), "archive", "--format=tar", "HEAD~1", "skill"]) 
        .stdout(std::process::Stdio::piped())
        .spawn()
        .unwrap();
    let status = Command::new("tar")
        .args(["-x", "--strip-components", "1", "-C", dest.to_str().unwrap()])
    ...
    let head = String::from_utf8(Command::new("git").args(["rev-parse", "HEAD"]).current_dir(&cache).output().unwrap().stdout).unwrap();
    ...
    let out = cmd.current_dir(&project).env("SK_CACHE_DIR", cache_root.to_str().unwrap()).args(["upgrade", "--all"]).output().unwrap();

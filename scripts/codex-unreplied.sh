...
| any(.codex_present and (.non_codex_present | not))
JQ
)
...
for pr in "${prs[@]}"; do
  ...
  json=$(gh api graphql -f query="$codex_threads_query" -F owner="$owner" -F name="$name" -F number="$pr")

  echo "$json" | jq --argjson codex "$codex_jq" "$unreplied_filter" \
    | jq -r 'if . then ("PR #" + (.pr|tostring) + " -> " + .url), (.pending[] | "  • " + .path + " — " + .first.author + " — " + .first.url + "\n    " + .first.excerpt) else empty end'

  if echo "$json" | jq --argjson codex "$codex_jq" "$has_pending_filter" | grep -q true; then
    exit_code=1
  fi
done
